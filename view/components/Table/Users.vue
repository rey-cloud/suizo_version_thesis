...
const toggleStatus = async (client) => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:8000/api/status/${client.id}`, {
      method: 'PUT',
    }); 
...
// Delete user
const deleteAction = async (client) => {
  name.value = 'delete_1';
  if (confirm('Delete this user?') === true) {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/delete/${client.username}`, {
        method: 'DELETE',
      }); 
